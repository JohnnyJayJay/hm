...
*** Glattheit
Wenn $f : \mathbb{R} \to \mathbb{C}$ [[*Stückweise Stetigkeit und Glattheit][stückweise glatt]], $f$ sowie $f'$ [[*Absolute Integrierbarkeit][absolut integrierbar]] und $f$ höchstens endlich viele [[*Stetigkeit][unstetige]] Punkte, dann ist $f$ beschränkt und
...
- Über den [[*Mittelwertsatz][Mittelwertsatz]]
...
*** Komplexe Funktionen
Spezialfall: Komplexe Funktionen. Wie zu erwarten ist dort $f'(x) = Re'(f)(x) + i Im'(f)(x)$.
*** Partielle Differenzierbarkeit
...
Eine Funktion $f$ ist absolut integrierbar, wenn
...
*** Komplexe Integrale
Für komplexwertige Funktionen $f$:
...
$f, g \in R(B)$ und $g \leq f$ auf $B$, dann ist

$$M_{f,g} := \{(x, y) \in B \times \mathbb{R} \; | \; g(x) \leq y \leq f(x)\}$$
...
:Beispiel:
Beispiel: $y'(x) + \frac{y(x)}{1 + x} + (1 + x)y^4(x) = 0$ auf dem Intervall $(-1, \infty)$.

Komponenten identifizieren: $g(x) = \frac{1}{1 + x}$, $h(x) = (1 + x)$, $\alpha = 4$.

[[*Lineare Differentialgleichungen][Lineare Dgl.]] aufstellen und lösen:
$$z'(x) = -(1 - \alpha)g(x)z(x) - (1 - \alpha)h(x) = -(1 - 4)\frac{1}{1 + x}z(x) - (1 - 4)h(x) = \frac{3}{1 + x}z(x) + 3(1 + x)$$
$\int g(x) dx$ berechnen (für homogene Lösung):
$$\int 3g(x)dx = \int \frac{3}{1 + x} dx = 3 \int \frac{1}{1 + x} dx = 3 \log (1 + x)$$
Homogene Lösung:
$$z_h(x) = ce^{\int g(x) dx} = ce^{3\log(1 + x)} = c(1 + x)^3, \; c \in \mathbb{R}$$
Variation der Konstanten:
\begin{align}
c(x) &= \int \frac{3h(x)}{z_h(x)} dx\\
&= \int \frac{3(1 + x)}{c (1 + x)^3} dx\\
&= \int \frac{3}{c(1 + x)^2} dx\\
&= \frac{3}{c} \int \frac{1}{(1 + x)^2} dx\\
&= \frac{3}{c} \int \frac{1}{t^2}dt \; (t = (1 + x), \frac{dt}{dx} = 1 \Leftrightarrow dx = 1 dt) \\
&= -\frac{3}{ct} = -\frac{3}{c(x + 1)}
\end{align}
Daraus folgt die partikuläre Lösung
$$z_p(x) = c(x)z_h(x) = - \frac{3}{c(x + 1)} \cdot c(1 + x)^3 = -3 (1 + x)^2$$
Zusammenführen:
$$z(x) = z_p(x) + z_h(x) = -3(1 + x)^2 + c(1 + x)^3$$

Lösung für ursprüngliche Bernoulli-Dgl.:
$$y(x) = z(x)^{\frac{1}{1 - 4}} = ((1 + x)^2 + c(1 + x)^3)^{-\frac{1}{3}}$$
:END:
...
Ein *lineares Differentialgleichungssystem* sieht wie folgt aus:

\begin{align}
& y_1'(x) = a_{1,1}y_1(x) + \dots + a_{1,n}y_n(x) + b_1(x) \\
& \vdots \\
& y_n'(x) = a_{n,1}y_1(x) + \dots + a_{n,n}y_n(x) + b_n(x)
\end{align}

Also: $n$ von einander abhängige [[*Lineare Differentialgleichungen][lineare Dgl.]] - dabei $b_j \in C(I, \mathbb{R})$ und $a_{i,j} \in \mathbb{R}$.

Man kann es auch einfacher wie in LA schreiben: Dafür

$$y(x) = \begin{pmatrix}y_1(x) \\ \vdots \\ y_n(x) \end{pmatrix} \in \mathbb{R}^n, A = \begin{pmatrix}
a_{1,1} & \dots & a_{1,n}\\
\vdots & & \vdots \\
a_{n, 1} & \dots & a_{n, n}
\end{pmatrix} \in \mathbb{R}^{n \times n}, b(x) = \begin{pmatrix}b_1(x) \\ \vdots \\ b_n(x)\end{pmatrix} \in \mathbb{R}^n$$

=>

$$y'(x) = Ay(x) + b(x)$$

Wenn $b = 0$, ist das System /homogen/, ansonsten ist es /inhomogen/.

Ebenfalls wie in LA bildet der Lösungsraum für ein homogenes System einen reellen Vektorraum (mit Dimension $n$), und allgemeine Lösungen für inhomogene Systeme lassen sich schreiben als "Allgemeine Lösung für zugehöriges homogenes System + Spezielle Lösung für inhomogenes System".

*AWPs mit inhomogenen Systemen haben immer genau eine Lösung.*

** Homogenes System Lösen

1. Eigenwerte von $A$ bestimmen
   1. Charakteristisches Polynom berechnen: $p_A(X) = \det (A - XI_n)$ ($I_n$ ist die $n \times n$ Einheitsmatrix)
   2. Nullstellen von $p_A(X)$ inkl. algebraische Vielfachheiten bestimmen
2. Bestimme für alle Eigenwerte (*für komplexe Eigenwerte nur jeweils einen der beiden komplex konjugierten*) $\lambda_j$ eine Basis von $V_j = \ker (A - \lambda_jI_n)^{k_j}$, wobei $k_j$ die algebraische Vielfachheit von $\lambda_j$ ist.
   Dabei kann man zuerst $\ker (A - \lambda_jI_n)$ berechnen, dann Basis zu $\ker (A - \lambda_jI_n)^2$ ergänzen usw.
3. Für jeden Eigenwert $\lambda_j$ und jeden Basisvektor $v \in V_j$ ist
   $$y(x) = e^{\lambda_jx} \left(v + \frac{x}{1!}(A - \lambda_jI_n)v + \dots + \frac{x^{k_j - 1}}{(k_j - 1)!}(A - \lambda_jI_n)^{k_j - 1}v \right) $$
   eine Lösung für das System (wenn $\lambda_j \in \mathbb{R}$) bzw. zwei Lösungen für das System $Im(y), Re(y)$ wenn $\lambda_j \in \mathbb{C} \setminus \mathbb{R}$.
4. Alle Lösungen $y^{(i)} : \mathbb{R} \to \mathbb{R}^n$ aus 3. zusammen sind ein *Fundamentalsystem* (Basis des Lösungsraums des Differentialgleichungssystems)

** Inhomogenes System Lösen

1. Löse das zugehörige homogene System $y'(x) = Ay(x)$
2. Füge die Lösungen aus dem Fundamentalsystem als Spalten zu einer Matrix zusammen
   $$Y(x) := (y^{(1)}(x), \dots , y^{(n)}(x))$$
3. Bestimme spezielle Lösung $y_p(x) = Y(x)c(x)$.
   Dabei ist $c(x) = \int (Y(x))^{-1}b(x) dx$
4. Füge spezielle und homogene Lösungen zusammen $\{y_p + y_h | y_h \in \{y^{(1)}, \dots, y^{(n)}\}\}$

* Lineare Differentialgleichungen n. Ordnung
Gegeben eine auf $I \subseteq \mathbb{R}$ (Intervall) $n$ mal [[Differentialrechnung][differenzierbare]] Funktion $y : I \to \mathbb{R}$ und Zahlen $a_0, \dots, a_{n-1} \in \mathbb{R}$, sowie $a_n = 1$:

$$Ly := \sum\limits_{k=0}^{n} a_ky^{(k)} = a_0y + a_1y' + \dots + a_{n - 1}y^{(n - 1)} + y^{(n)}$$

$Ly$ ist also eine Funktion $I \to \mathbb{R}$.

$$(Ly)(x) = b(x)$$

ist eine *lineare Dgl. n. Ordnung*. Falls $b = 0$ ist sie *homogen*, ansonsten *inhomogen*.

Lösungen verhalten sich genau wie bei [[*Lineare Systeme mit Konstanten Koeffizienten][linearen Systemen]].

** Homogene Dgl. Lösen
$$(Ly)(x) = 0$$

1. /Charakteristisches Polynom der Dgl./ aufstellen:
   $$p(X) = \sum_{k=0}^{n} a_kX^k = a_0 + a_1 X + \dots + a_{n - 1}X^{n - 1} + X^n$$
2. Nullstellen $\lambda_j$ und algebraische Vielfachheiten $k_j$ berechnen (wie bei [[*Lineare Systeme mit Konstanten Koeffizienten][linearen Systemen]] - d.h. jeweils nur eine der beiden zusammengehörigen komplexen Nullstellen, falls vorhanden)
3. Für alle Nullstellen $\lambda_j$:
   - Falls reell: $e^{\lambda_jx}, xe^{\lambda_jx}, \dots, x^{k_j - 1}e^{\lambda_jx}$ sind Lösungen
   - Falls komplex: $\lambda_j = \alpha + i \beta$
     $e^{\alpha x} \cos \beta x, \; x e^{\alpha x} \cos \beta x, \dots , \; x^{k_j - 1}e^{\alpha x} \cos \beta x$
     und
     $e^{\alpha x} \sin \beta x, \; x e^{\alpha x} \sin \beta x, \dots, \; x^{k_j - 1}e^{\alpha x} \sin \beta x$
     sind Lösungen
4. Alle Lösungen aus 3. zusammen bilden eine Basis für den Lösungsraum der Dgl.

** Inhomogene Dgl. Lösen
$$(Ly)(x) = b(x)$$

1. Voraussetzung: $b(x) = q(x)e^{\gamma x}\cos (\delta x)$ *oder* $b(x) = q(x)e^{\gamma x}\sin(\delta x)$ ($q(x)$ Polynom von Grad $m$, $\gamma, \delta \in \mathbb{R}$) - $b$ muss von dieser Form sein
2. Charakteristisches Polynom $p$ der zugehörigen homogenen Dgl. $(Ly)(x) = 0$ berechnen
3. Fallunterscheidung:
   - $p(\gamma + i \delta) \neq 0$ (keine Nullstelle):
     $$y_p(x) := (\hat{q}(x) \cos(\delta x) + \tilde{q}(x) \sin(\delta x)) e^{\gamma x}$$
   - $p(\gamma + i \delta) = 0$ (Nullstelle mit Vielfachheit $v$):
     $$y_p(x) := x^v (\hat{q}(x) \cos (\delta x) + \tilde{q}(x) \sin(\delta x)) e^{\gamma x}$$
4. $\hat{q}$ und $\tilde{q}$ sind Polynome von Grad $m$ (also gleicher Grad wie $q$ in $b(x)$)

* Fouriertransformation

** Stückweise Stetigkeit und Glattheit
Gegeben Funktion $g : [a,b] \to \mathbb{R}$.

*Stückweise stetig:* $[a, b]$ kann in $m$ direkt nebeneinander liegende (z.B. $(1, 2), (2, 4)$) offene Intervalle aufgeteilt werden (1. Intervall fängt bei $a$ an, letztes Intervall hört bei $b$ auf), sodass $g$ auf diesen Intervallen stetig ist.

*Stückweise glatt:* Das gleiche wie stückweise stetig, nur mit Teilintervallen, auf denen $g$ /stetig differenzierbar/ ist.

In diesen Fällen existieren zu jedem Teilintervall die Grenzwerte Richtung Anfang und Ende, also $g(t_j+), g(t_{j + 1}-)$ für alle Teilintervalle $(t_j, t_{j+1})$. Für stückweise glatte Funktionen existieren auch die jeweiligen Grenzwerte der Ableitung.

*Stückweise stetig/glatt auf $\mathbb{R}$ = Auf jedem Intervall $[a, b] \subseteq \mathbb{R}$ stückweise stetig/glatt*

Die Ableitung einer stückweise glatten Funktion kann man für jeden Punkt $x \in (a, b)$ definieren, selbst wenn $g$ dort nicht differenzierbar ist:

$$g'(x) = \frac{1}{2}(g'(x+) + g'(x-))$$

#+DOWNLOADED: screenshot @ 2021-09-10 18:54:05
[[file:Fouriertransformation/2021-09-10_18-54-05_screenshot.png]]

:Beispiel:
Beispiel: $g(x) = |x|$

Stückweise glatt auf $\mathbb{R}$, jeweils mit den Intervallen $[a, 0)$ und $(0, b]$. $g'(0-) = -1, g'(0+) = 1 \Rightarrow g'(x) = 0$
:END:

** Definition
Gegeben eine [[*Absolute Integrierbarkeit][absolut integrierbare]] und stückweise stetige Funktion $f : \mathbb{R} \to \mathbb{C}$ ist
...
Es gilt:
- $\hat{f}$ ist beschränkt
- $\lim_{s \to \pm \infty} \hat{f}(s) = 0$
- $\hat{f}$ ist stetig

** Cauchyscher Hauptwert

Gegeben: Funktion $f : \mathbb{R} \to \mathbb{C}$, integrierbar auf jedem $[a, b] \subseteq \mathbb{R}$. Sofern definiert:

$$CH - \int_{-\infty}^{\infty} f(x)dx := \lim\limits_{\alpha \to \infty} \int_{-\alpha}^{\alpha} f(x)dx$$

Dies ist der *Cauchysche Hauptwert* von $f$.

Wenn $\int_{-\infty}^{\infty} f(x) dx$ [[*Uneigentliche Integrale][konvergiert]], dann ist das gleich dem Cauchyschen Hauptwert.

Wenn $f$ stückweise glatt und absolut integrierbar:

$$CH- \int_{-\infty}^{\infty} \hat{f}(s) e^{ist}ds = \frac{1}{2} (f(t+) + f(t-))$$

und falls $f$ außerdem stetig

$$CH- \int_{-\infty}^{\infty} \hat{f}(s)e^{ist}ds = f(t)$$
