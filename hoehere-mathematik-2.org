...
3. Für Eigenwert $\lambda_j$ und Basisvektor $v \in V_j$ ist
...
1. Voraussetzung: $b(x) = q(x)e^{\gamma x}\cos (\delta x)$ ($q(x)$ Polynom von Grad $m$, $\gamma, \delta \in \mathbb{R}$) - $b$ muss von dieser Form sein
