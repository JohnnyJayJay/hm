...
In anderen Worten: Jede Folge von Eingangswerten nähert sich, eingesetzt in $f$ an $a$ an.
...
$D_{\delta}(x_0)$ steht hier für die /Delta-Umgebung von $x_0$/ $U_{\delta}(x_0)$ geschnitten mit dem Definitionsbereich $D$ der Funktion ohne $x_0$ selbst, also für alle Eingangswerte in der Nähe von $x_0$, die im Definitionsbereich von $f$ liegen.

Die Definition bedeutet im Prinzip:
*Für jedes noch so kleine $\epsilon$ finde ich einen Radius $\delta$ um $x_0$, sodass der Abstand aller Funktionswerte von $a$ in diesem Radius kleiner als $\epsilon$ ist.*
...
**** Kombinationen
Wie auch Folgen oder Reihen übertragen sich Kombinationen von Funktionen auf deren Grenzwerte:
...
**** Stetige Funktionen
Der Grenzwert einer stetigen Funktion $f$ (s. nächster Abschnitt) in $x_0$ muss gleich $f(x_0)$ sein, sofern definiert. Man kann in solchen Fällen einfach direkt $x_0$ einsetzen, um den Grenzwert zu berechnen.
